                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="overflow-x-auto">
                        ...
                        <form action="{{ route('admin.navigations.update', $navigation->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-4">
                                <label for="announcement_for" class="block text-sm font-medium text-gray-700">Navigation For</label>
                                <select name="announcement_for" id="announcement_for" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                    ...
                                    <option value="main_website" {{ old('announcement_for', $navigation->announcement_for) === 'main_website' ? 'selected' : '' }}>Main Website</option>
                                    @foreach ($wings as $wing)
                                        <option value="{{ $wing->id }}" {{ old('announcement_for', $navigation->announcement_for) == $wing->id ? 'selected' : '' }}>
                                            {{ $wing->title }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="datetime-local" name="starts_at" id="starts_at" value="{{ old('starts_at', optional($navigation->starts_at)->format('Y-m-d\TH:i')) }}"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            </div>

                            <div class="mb-4">
                                <label for="ends_at" class="block text-sm font-medium text-gray-700">Ends At</label>
                                <input type="datetime-local" name="ends_at" id="ends_at" value="{{ old('ends_at', optional($navigation->ends_at)->format('Y-m-d\TH:i')) }}"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            </div>

                            <div class="mb-4 flex items-center">
                                <input type="checkbox" name="is_active" id="is_active" {{ old('is_active', $navigation->is_active) ? 'checked' : '' }}
